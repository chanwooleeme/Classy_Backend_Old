...
#!/bin/bash
PROJECT_NAME="apps/deploy"
JAR_PATH="/home/ec2-user/apps/deploy/classy-api/build/libs/*.jar"
...
echo "> $DEPLOY_JAR" >> $DEPLOY_LOG_PATH

nohup java -jar -Dspring.profiles.active=beta $DEPLOY_JAR --server.port=8080 >> $APPLICATION_LOG_PATH 2> $DEPLOY_ERR_LOG_PATH &
